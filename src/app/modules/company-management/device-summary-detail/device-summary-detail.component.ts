...
@Component({
  selector: "app-device-summary-detail",
  templateUrl: "./device-summary-detail.component.html",
  styleUrls: ["./device-summary-detail.component.scss"],
})
export class DeviceSummaryDetailComponent implements OnInit {
  ...
  constructor(
    ...
    this.cardText =
      this.formBgColor === "#ffffff" ? "#000000" : this.formBgColor;
  }
  ...
  settings(device: DataDetails) {
    const modalRef = this.modal.open(DeviceSummaryDetailsSettingsComponent, {
      size: "lg",
    });
    (
      modalRef.componentInstance as DeviceSummaryDetailsSettingsComponent
    ).device = device;
  }

  dataDetails(device: DataDetails) {
    const modalRef = this.modal.open(DeviceSummaryDataDetailComponent, {
      size: "lg",
    });
    (modalRef.componentInstance as DeviceSummaryDataDetailComponent).device =
      device;
    (modalRef.componentInstance as DeviceSummaryDataDetailComponent).companyId =
      this.companyId;
  }
        ...
        (value) => {
          ...
          this.devices = value.map((each, index) => {
            ...
            if (each.tab6) {
              ...
              //       type: "geolocation",
              //       data: { showMap: true, address, latitude, longitude },
              //     });
              //   });
  ...
  filterDevices = () => {
    ...
    if (network === undefined || network === "ALL") delete filter.network;
    ...
    //   this.devices = this.devicesConst.filter(
