...
@Component({
  selector: "app-corporate-user-update",
  templateUrl: "./corporate-user-update.component.html",
  styleUrls: ["./corporate-user-update.component.scss"],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CorporateUserUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.source$ = this.route.paramMap.pipe(
      concatMap((params) => {
        return this._resolve(params.get("user"));
      })
    );
  }
  ...
  private _resolve(
    ...
    return zip(
      this.dataAccessor
        ...
        .findUserById(userId),
      this.groupDataAccessor.fetchForUser(this.company?.companyId, userId)
    ).pipe(
